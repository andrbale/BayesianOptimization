...
def acq_max(ac, gp, y_max, bounds, random_state, n_warmup=10000, n_iter=10):
    ...
    for x_try in x_seeds:
        ...
        if max_acq is None or -np.squeeze(res.fun) >= max_acq:
            x_max = res.x
            max_acq = -np.squeeze(res.fun) #for new scipy version

    # Clip output to make sure it lies within the bounds. Due to floating
    # point technicalities this is not always the case.
    return np.clip(x_max, bounds[:, 0], bounds[:, 1])

###andrbale### add return of acquisition function value
def acq_max_terminal(ac, gp, y_max, bounds, random_state, n_warmup=10000, n_iter=10):
    """
    A function to find the maximum of the acquisition function
    ...
    # Clip output to make sure it lies within the bounds. Due to floating
    # point technicalities this is not always the case.
    return np.clip(x_max, bounds[:, 0], bounds[:, 1]), ac(res.x.reshape(1, -1), gp=gp, y_max=y_max)[0]
